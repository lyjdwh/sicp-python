...
def check_func(func, tests, comp=lambda x, y: x == y, in_print=repr, out_print=repr):
...
def check_for_updates(index, filenames, version):
    ...
    try:
        ...
        return
    remote_version = re.search("__version__ = '(.*)'", remotes[filenames[0]])
...
def run_tests(name, remote_index, autograder_files, version, **kwargs):
    ...
    parser.add_argument(
        "-q", "--question", type=str, help="Run tests for the specified question"
    )
    parser.add_argument(
        "-v",
        "--version",
