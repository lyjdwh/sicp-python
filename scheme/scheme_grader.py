...
from scheme import LambdaProcedure, Pair, create_global_frame, nil, read_line
...
try:
    ...
    print(
        "Unfortunately, the autograder cannot run because "
...
def problem_2(grades):
    ...
    tests2 = [
        ("(a b . c d)", "Error"),
        ("((a b) (c (d (e f))))", pairify([["a", "b"], ["c", ["d", ["e", "f"]]]])),
...
def problem_A5(grades):
    tests1 = [
        ...
        ("(define pi 3.14159) (define radius 10) (* pi (* radius radius))", 314.159),
    ]
    if check_func(scheme_eval, tests1, comp=scheme_equal):
        return True
...
def problem_8(grades):
    tests1 = [
        (
            "(lambda (x y) (+ x y))",
            LambdaProcedure(
                pairify(["x", "y"]), pairify(["+", "x", "y"]), create_global_frame()
            ),
        )
    ]
    tests2 = [
...
def problem_A9(grades):
    tests1 = [
        ...
        (
            ...
            LambdaProcedure(
                pairify(["x"]), pairify(["*", "x", "x"]), create_global_frame()
            ),
        ),
        (
            "(begin (define (f x) 1 2) f)",
            LambdaProcedure(
                pairify(["x"]), pairify(["begin", 1, 2]), create_global_frame()
            ),
        ),
...
def problem_B11(grades):
    ...
    tests1 = [
        (
            "(lambda (x y z) x)",
            LambdaProcedure(pairify(["x", "y", "z"]), "x", create_global_frame()),
        ),
...
def scheme_equal(x, y):
    ...
    if type(x).__name__ == "nil" and type(y).__name__ == "nil":
        ...
        type(x).__name__ == "LambdaProcedure" and type(y).__name__ == "LambdaProcedure"
    ):
...
project_info = {
    ...
    "autograder_files": ["scheme_grader.py", "scheme_test.py", "autograder.py",],
    "version": __version__,
}
