...
class AntsGUI(object):
    ...
    def _init_control_panel(self, colony):
        ...
        for name, ant_type in colony.ant_types.items():
            ...
            cost_pos = shift_point(
                panel_pos,
                (width / 2, ANT_IMAGE_HEIGHT + 4 + PANEL_PADDING[1]))
        ...
        for name, place in colony.places.items():
            ...
            def on_click(colony, frame, name=name):
                ...
                if ant_type is 'Remover':
                    ...
                    try:
                        print("colony.deploy_ant('{0}', '{1}')".format(
                            name, ant_type))
    ...
    def _throw(self, ant, colony):
        ...
        if bee:
            start = shift_point(self.place_points[ant.place.name],
                                LEAF_START_OFFSET)
            end = shift_point(self.place_points[bee.place.name],
                              LEAF_END_OFFSET)
...
def leaf_coords(pos, angle, length):
    ...
    return [
        graphics.translate_point(pos, a, d) for a, d in zip(angles, distances)
    ]
